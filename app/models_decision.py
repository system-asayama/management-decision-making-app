...
class RestructuredPL(Base):
    ...
    # 1. 売上高
    sales = Column(BigInteger, default=0, nullable=False)
    # 2. 売上原価
    cost_of_sales = Column(BigInteger, default=0, nullable=False)
    # 売上原価内訳
    beginning_inventory = Column(BigInteger, default=0, nullable=False)      # 期首棚卸高（合計）
    # 期首棚卸高 内訳
    ...
    ending_inventory = Column(BigInteger, default=0, nullable=False)          # 期末棚卸高（合計）
    # 期末棚卸高 内訳
    ...
    external_cost_adjustment = Column(BigInteger, default=0, nullable=False)
    # 粗付加価値（売上総利益＋外部経費調整）
    gross_added_value = Column(BigInteger, default=0, nullable=False)
    # 3. 販売費及び一般管理費
    selling_general_admin_expenses = Column(BigInteger, default=0, nullable=False)
    # 販管費内訳
    ...
    capital_regeneration_cost = Column(BigInteger, default=0, nullable=False) # 資本再生費（減価償却費＋修繕費）
    ...
    # 営業利益
    operating_income = Column(BigInteger, default=0, nullable=False)
    # 4. 営業外損益
    financial_profit_loss = Column(BigInteger, default=0, nullable=False)     # 金融損益（受取利息－支払利息）
    other_non_operating = Column(BigInteger, default=0, nullable=False)       # その他の損益
    # 経常利益
    ordinary_income = Column(BigInteger, default=0, nullable=False)
    # 5. 特別損益
    extraordinary_profit_loss = Column(BigInteger, default=0, nullable=False) # 特別損益合計
    # 税引前当期純利益
    income_before_tax = Column(BigInteger, default=0, nullable=False)
    # 法人税等
    income_taxes = Column(BigInteger, default=0, nullable=False)
    # 当期純利益
    ...
    # 旧フィールド（後方互換性）
    non_operating_income = Column(BigInteger, default=0, nullable=False)
    non_operating_expenses = Column(BigInteger, default=0, nullable=False)
    extraordinary_income = Column(BigInteger, default=0, nullable=False)
    extraordinary_loss = Column(BigInteger, default=0, nullable=False)
...
class RestructuredBS(Base):
    ...
    fixed_assets = Column(BigInteger, default=0, nullable=False)
    # 資産合計
    total_assets = Column(BigInteger, default=0, nullable=False)
    ...
    # 固定負債合計
    fixed_liabilities = Column(BigInteger, default=0, nullable=False)
    # 負債合計
    total_liabilities = Column(BigInteger, default=0, nullable=False)
    ...
    # 負債純資産合計
    total_liabilities_and_net_assets = Column(BigInteger, default=0, nullable=False)
    
    # 脈診注記
    discounted_notes_note = Column(BigInteger, default=0, nullable=False)     # 割引手形高（脈診）
    endorsed_notes_note = Column(BigInteger, default=0, nullable=False)       # 裏書手形高（脈診）

...
def has_inner_space(name):
    """文字間にスペースがある場合True（例：「給 料 手 当」）"""
...
try:
    ...
    for item in db.query(PlAccountItem).all():
        if has_inner_space(item.account_name):
            db.query(PlStatementValue).filter_by(account_item_id=item.id).delete()
            db.delete(item)
            deleted_pl += 1
            print(f"[PL削除] id={item.id}, name='{item.account_name}'")

    for item in db.query(BsAccountItem).all():
        if has_inner_space(item.account_name):
    ...
    for item in db.query(McrAccountItem).all():
        if has_inner_space(item.account_name):
